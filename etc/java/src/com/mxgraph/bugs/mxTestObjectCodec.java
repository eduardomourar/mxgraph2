/**
 * $Id: mxTestObjectCodec.java,v 1.1 2012/11/15 13:26:39 gaudenz Exp $
...
public class mxTestObjectCodec
{
	 ...
	 */
	public mxTestObjectCodec(Object template)
	{
	 ...
	 */
	public mxTestObjectCodec(Object template, String[] exclude,
	...
	{
		Node node = enc.getDocument().createElement(getName());
			...
			String attr, Object value, Node node)
	{
		...
		{
			Node child = enc.getDocument().createElement("add");
		...
		if (value != null && !value.equals(template))
		{
			...
			else if (obj instanceof Collection)
			{
						...
						&& types[0] instanceof ParameterizedType)
				{
					...
					System.out.println("type=" + type + " class=" + clazz);
				}
				else
				{
			...
			if (name != null)
			{
				try
				{
					Node xml = mxUtils.loadDocument(
							mxTestObjectCodec.class.getResource(name)
