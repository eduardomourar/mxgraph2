...
mxRadialTreeLayout.prototype.execute = function(parent, root)
{
	...
	var centerX = rootBounds.x + rootBounds.width / 2;
	var centerY = rootBounds.y + rootBounds.height / 2;
	...
	while (child != null)
	{
		...
		this.setVertexLocation(cell, centerX - vertexBounds.width / 2 + this.firstRadius * Math.cos(theta), centerY + this.firstRadius * Math.sin(theta));
		row.push(child);
		child = child.next;
	}
	...
	if (prevRow.length > 0)
	{
		this.layerN(row, this.firstRowMinX, this.firstRowMaxX);
	}
		...
		if (nodes[i].child != null)
		{
			...
			while (child != null)
			{
				...
				if (nextRowMinX == null)
				{
					...
					nextRowMaxY = vertexBounds.x + vertexBounds.width;
				}
				else
				{
					nextRowMinX = Math.min(vertexBounds.x, nextRowMinX);
					nextRowMaxY = Math.max(vertexBounds.x + vertexBounds.width - prevRowX, nextRowMaxY);
				}
